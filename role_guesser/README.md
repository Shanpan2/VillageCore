## はじめに
本Botは非公式のファンプロジェクトであり、Among Us および InnerSloth とは一切関係ありません。
本プロジェクトはこれらの公式から承認・支援・提携を受けていません。
本リポジトリに含まれる一部の名称・概念は、InnerSloth LLCに帰属します。©InnerSloth LLC.

また本プロジェクトは各MODの開発者様とは直接関係ありません。
問題がある場合は対応いたしますのでご連絡ください。
